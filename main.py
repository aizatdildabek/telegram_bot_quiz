...
def callback_handler(call):
    ...
    if state.current_question < len(questions):
        ...
        end_time = time.time()
        sub_time = round(end_time - state.start_time) // 60
        bot.send_message(user_id, text=f"Тест завершен, вы ответили {state.correct_answer} вопросов правильно из {len(questions)} вопросов на {sub_time} время")
